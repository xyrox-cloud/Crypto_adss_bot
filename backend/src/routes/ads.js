...
// ── POST /api/ads/claim (Frontend Ad Claim) ──────────────────────────────────
router.post('/claim', extractTelegramUser, rewardLimiter, (req, res) => {
  try {
    const db = getDb();
    const telegramId = req.telegramUser.id;

    const user = db.prepare('SELECT id, balance, total_earned, total_ads_watched, banned FROM users WHERE telegram_id = ?').get(telegramId);
    if (!user) return res.status(400).json({ error: 'User not found' });
    if (user.banned) return res.status(403).json({ error: 'Account is banned' });
    ...
    if (todayCount >= maxAdsPerDay) return res.status(429).json({ error: `Daily ad limit reached (${maxAdsPerDay}/day)` });

    const maxAdsPerHour = Math.floor(settings.max_ads_per_hour ?? 5);
    const hourCount = db.prepare(`SELECT COUNT(*) as count FROM ad_watches WHERE user_id = ? AND timestamp >= datetime('now', '-1 hour')`).get(user.id).count;
    if (hourCount >= maxAdsPerHour) return res.status(429).json({ error: `Hourly ad limit reached (${maxAdsPerHour}/hour)` });

    if (cooldownSecs > 0) {
      const lastWatch = db.prepare(`SELECT timestamp FROM ad_watches WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1`).get(user.id);
      if (lastWatch) {
        const secsSinceLast = (Date.now() - new Date(lastWatch.timestamp).getTime()) / 1000;
        if (secsSinceLast < cooldownSecs) {
          const waitSecs = Math.ceil(cooldownSecs - secsSinceLast);
    ...
    const watchTx = db.transaction(() => {
      const watchInfo = db.prepare(`INSERT INTO ad_watches (user_id, telegram_id, reward_amount, platform_cut, ip_address, source) VALUES (?, ?, ?, ?, ?, ?)`).run(user.id, telegramId, userShare, platformCut, ipAddress, 'frontend_claim');
      db.prepare(`INSERT INTO ad_rewards (user_id, amount, ip) VALUES (?, ?, ?)`).run(user.id, userShare, ipAddress);
      db.prepare(`UPDATE users SET balance = balance + ?, total_earned = total_earned + ?, total_ads_watched = total_ads_watched + 1, last_seen = CURRENT_TIMESTAMP WHERE id = ?`).run(userShare, userShare, user.id);
      db.prepare(`INSERT INTO platform_revenue (ad_watch_id, amount) VALUES (?, ?)`).run(watchInfo.lastInsertRowid, platformCut);
      db.prepare(`INSERT INTO activity_log (action, details) VALUES (?, ?)`).run('ad_reward_granted', `User ${telegramId} earned ${userShare} TON via frontend`);
      return watchInfo.lastInsertRowid;
    });
    ...
    res.status(200).json({ success: true, reward: userShare });
  } catch (err) {
    console.error('Frontend ad claim error:', err);
